...
static int mydrv_major = MYDRV_MAJOR;
module_param(mydrv_major, int, S_IRUGO);
...
// ioctl 64 位函数定义
//
// compat_ioctl 这个函数是为了使32位的应用程序可以与64位的内核驱动兼容而产生的。 
// 最主要实现的功能就是：
//   将应用层32位的数据转成64位传给64位内核驱动。
//   将内核驱动64位的数据转成32位传给32位内核驱动。
//
// 所以调用关系如下：
//   当应用层是32位程序，内核及架构是32位程序，那么驱动的 unlocked_ioctl 函数被调用。
//   当应用层是32位程序，内核及架构是64位程序，那么驱动的 compat_ioctl 函数被调用。
//   当应用层是64位程序，内核及架构是64位程序，那么驱动的 unlocked_ioctl 函数被调用。
//
//------------------------------------------------------------------------------
...
static inline int mydrv_init(void)
{
    ...
    mydrv_devno = MKDEV(mydrv_major, 0);
    if ( mydrv_major ) {
        // 静态申请设备号
